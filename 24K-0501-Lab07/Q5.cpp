...
void quickSort(int arr[], int start, int end, string method) {
    if (start < end) {
        ...
        if (method == "first") pivotIndex = start;
        else if (method == "random") pivotIndex = start + rand() % (end - start + 1);
        else if (method == "middle") pivotIndex = start + (end - start) / 2;
        else if (method == "median3") {
            int mid = start + (end - start) / 2;
            int a = arr[start], b = arr[mid], c = arr[end];
            if ((a > b) != (a > c)) pivotIndex = start;
